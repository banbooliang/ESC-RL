...
class BLIP_Decoder(nn.Module):
    def __init__(self,                 
        ...
        super().__init__()
        self.args = args
        self.annotation = json.load(open('/data/lgy/report_generation/REVTAF-main/data/mimic_cxr/mimic_annotation_promptmrg.json','r'))['train']
        vision_width = 2048
        ...
        with open('/data/lgy/report_generation/REVTAF-main/data/mimic_cxr/image_region_score_train.json', 'r') as f:
    ...
    def generate(self, image, clip_memory, region_txt, region_image, sample=False, num_beams=3, max_length=100, min_length=10, top_p=0.9, repetition_penalty=1.0):
        ...
        for b in range(image.shape[0]):
            t_path = os.path.join('/data/lgy/report_generation/REVTAF-main/data/mimic_cxr', 'medclip_txt_embeddings', self.annotation[index_select[b]]['image_path'][0]).replace('.jpg', '.npy')
    ...
    @torch.no_grad()
    def generate_with_ids(self, image, clip_memory, region_txt, region_image,
                          sample=True, max_length=100, min_length=10,
                          num_beams=1, top_p=0.9, temperature=1.0,
                          repetition_penalty=1.0):
        """
        Only used in RL setting
        Return：
          - captions: list[str]，去掉 prompt 的文本
          - sequences: tensor [B, T]，包含起始 token、prompt 和生成 token 的完整 id 序列
        不修改现有的 generate()
        """
        # ===== 与 generate 相同的图像 + prompt + OT 部分 =====
        image_embeds, avg_embeds = self.visual_encoder(image) 
        # NxKxC -> KxNxC
        clip_memory = torch.permute(clip_memory, (1, 0, 2))
        query_embed = self.vision_proj(avg_embeds)
        ...
        for b in range(image.shape[0]):
            t_path = os.path.join(
                '/data/lgy/report_generation/REVTAF-main/data/mimic_cxr',
        ...
        # OT-based alignment
        region_txt_ = self.ot_txt_proj1(region_txt)
        enhance_region_embed, region_map = self.ot_cross_attention(image_embeds, region_txt_) 
        global_txt_ = self.ot_txt_proj2(global_txt)
        enhance_global_embed, global_map = self.ot_cross_attention(image_embeds, global_txt_)
        image_embeds = torch.cat([enhance_global_embed, enhance_region_embed], dim=-1)  

        # ===== decoder generate（注意这里走 do_sample=True）=====
        image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(image.device)
        model_kwargs = {
        ...
        gen_outputs = self.text_decoder.generate(
            input_ids=input_ids,
            min_length=min_length,
            max_new_tokens=max_length,
            do_sample=sample,                # RL 时启用随机采样
            top_p=top_p,
            temperature=temperature,
            num_beams=num_beams,             # 一般 RL 用 num_beams=1
            eos_token_id=self.tokenizer.sep_token_id,
            pad_token_id=self.tokenizer.pad_token_id,
            repetition_penalty=repetition_penalty,
            attention_mask=attn_masks,
        ...
        sequences = gen_outputs.sequences  # [B, T]
        ...
        return captions, sequences
    
    def compute_seq_logprob(self, image, clip_memory, region_txt, region_image, sequences):
        """
        给定图像 + 对应生成好的 token ids，计算每个样本整句 log p(y|x)。
        用于 policy gradient:  L = - (R - b) * log_prob
        注意：这里会重新跑一遍 encoder + decoder，有一定额外开销，但实现最干净。
        """
        # ===== 先计算 encoder 侧的 image_embeds（与 generate 一致）=====
        image_embeds, avg_embeds = self.visual_encoder(image) 
        clip_memory = torch.permute(clip_memory, (1, 0, 2))
        query_embed = self.vision_proj(avg_embeds)
        hs = self.memory(clip_memory, None, query_embed.unsqueeze(0), None)
        ...
        # 这里只需要 encoder hidden，不需要再算 cls 分支
        h_embedding_region = self.gwd.encode(region_image)
        h_embedding_ref = self.gwd.encode(self.r_i_score)
        hyper_dist = dist_matrix(h_embedding_region, h_embedding_ref, c=self.c)
        ...
        region_txt_ = self.ot_txt_proj1(region_txt)
        enhance_region_embed, _ = self.ot_cross_attention(image_embeds, region_txt_)
        global_txt_ = self.ot_txt_proj2(global_txt)
        enhance_global_embed, _ = self.ot_cross_attention(image_embeds, global_txt_)
        image_embeds = torch.cat([enhance_global_embed, enhance_region_embed], dim=-1)

        image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long, device=image.device)

        # ===== 构造 decoder 的输入和 label =====
        # sequences: [B, T]，来自 generate_with_ids 的输出
        # 典型 seq2seq：输入是去掉最后一个 token，label 是去掉第一个 token
        input_ids = sequences[:, :-1].contiguous()
        target_ids = sequences[:, 1:].contiguous()

        # padding 的位置不参与 loss / log_prob
        ...
        # 对应位置取出 target token 的 log_prob
        token_log_probs = log_probs.gather(-1, target_ids.unsqueeze(-1)).squeeze(-1)  # [B, T-1]

        # 对 padding / 无效 label 位置清零
        token_log_probs[target_ids == self.tokenizer.pad_token_id] = 0.0

        # 每个样本整句 log_prob = token log_prob 之和
        seq_logprob = token_log_probs.sum(dim=-1)  # [B]

        return seq_logprob
    
    def _build_prompt_from_cls(self, avg_embeds):
        cls_logits = self.cls_head(avg_embeds)
        cls_logits = cls_logits.view(-1, 4, 18)
        cls_probs = F.softmax(cls_logits, dim=1)
        cls_preds_logits = cls_probs[:, 1, :14]
        cls_preds = torch.argmax(cls_probs, dim=1).cpu().numpy().tolist()

        prompts = []
        for pred in cls_preds:
            prompt = ' '.join([self.tokenizer.additional_special_tokens[i] if i < 0 else ['[BLA]', '[POS]', '[NEG]', '[UNC]'][c] for i, c in enumerate(pred)])
            # fallback-safe explicit prompt construction
            prompt = ' '.join(['[BLA]' if c == 0 else '[POS]' if c == 1 else '[NEG]' if c == 2 else '[UNC]' for c in pred]) + ' '
            prompts.append(prompt)
    ...
    def _encode_for_generation(self, image, clip_memory, region_txt, global_txt=None, region_image=None):
        image_embeds, avg_embeds = self.visual_encoder(image)
        clip_memory = torch.permute(clip_memory, (1, 0, 2))
        query_embed = self.vision_proj(avg_embeds)
        hs = self.memory(clip_memory, None, query_embed.unsqueeze(0), None)
        ...
        region_txt = self.ot_txt_proj1(region_txt)
        enhance_region_embed, _ = self.ot_cross_attention(image_embeds, region_txt)
        global_txt = self.ot_txt_proj2(global_txt)
        enhance_global_embed, _ = self.ot_cross_attention(image_embeds, global_txt)
        image_embeds = torch.cat([enhance_global_embed, enhance_region_embed], dim=-1)
        image_atts = torch.ones(image_embeds.size()[:-1], dtype=torch.long).to(image.device)
        model_kwargs = {"encoder_hidden_states": image_embeds, "encoder_attention_mask": image_atts}
        return prompts, input_ids, attn_masks, model_kwargs, cls_preds, cls_preds_logits

    def _decode_report_only(self, sequences, prompts):
        reports = []
        for i, seq in enumerate(sequences):
            caption = self.tokenizer.decode(seq, skip_special_tokens=True)
            prompt = prompts[i]
    ...
    def sample_scst(
        ...
        """
        Return:
            sequences: generated token ids, shape [B, L]
            token_logprobs: log p(y_t | y_<t, x), aligned to sequences[:, 1:], shape [B, L-1]
            reports: decoded report texts without the 18-label prompt prefix
            cls_preds / cls_preds_logits: classification branch outputs
            prompts: the generated prompt prefix strings
        """
        prompts, input_ids, attn_masks, model_kwargs, cls_preds, cls_preds_logits = self._encode_for_generation(
            image=image,
        ...
        # Teacher-forced pass on generated sequence to get per-token log-probabilities.
        ...
        reports = self._decode_report_only(outputs, prompts)
        return outputs, token_logprobs, reports, cls_preds, cls_preds_logits, prompts
