...
class MedKLIPAdapter(nn.Module):

    def __init__(self, config, ana_book, disease_book, mode='train'):
        super(MedKLIPAdapter, self).__init__()

        self.mode = mode
        self.d_model = config.medklip_d_model
        # ''' book embedding'''
        with torch.no_grad():
            bert_model = self._get_bert_basemodel(config.medklip_text_decoder,freeze_layers = None).to(ana_book['input_ids'].device)
        ...
        self.dropout_feas = nn.Dropout(config.medklip_dropout)
