...
our_list = ['enlarged cardiomediastinum',
            'cardiomegaly',
            'lung opacity',
            'lung lesion',
            'edema',
...
def build_candidate_text(odict):
    """
    将 original_dict 构造成包含 key 和释义的长文本，供大模型判断。
    """
    txt = ""
    for key, definition in odict.items():
        txt += f"- {key}: {definition}\n"
    return txt

candidate_text = build_candidate_text(original_dict)
original_keys = list(original_dict.keys())

# LLM matching
def match_with_llm(query, candidates):
    prompt = f"""
        You are a medical expert performing semantic mapping.

        Your task: For the given medical label:
        **"{query}"**

        Find the best matching item from the following list of dictionary entries (each entry includes a key and its meaning):

        {candidate_text}

        Return ONLY the KEY (not the explanation).
        If multiple keys seem relevant, choose the MOST specific and medically appropriate one.
        """
...
for idx, item in enumerate(our_list):
    best_key = match_with_llm(item, candidate_text)
    if best_key not in original_keys:
        print(f"[Warning] LLM returned unknown key: {best_key}")
    else:
        orig_idx = original_keys.index(best_key)
        mapping[idx] = orig_idx
        print(f"{idx}: {item} → {best_key}  (index {orig_idx})")

print("\nFinal mapping dict:")
print(mapping)


## {0: 17, 1: 24, 2: 9, 3: 65, 4: 11, 5: 14, 6:20,7: 12, 8: 10, 9: 8, 10: 16, 11: 25, 12: 32}